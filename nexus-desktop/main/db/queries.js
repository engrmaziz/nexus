...
function getStatements() {
  const db = getDb();
  return {
    insertDownload:          db.prepare(`INSERT OR REPLACE INTO downloads (id, url, title, filename, save_path, category, status, file_size, downloaded, mime_type, referrer, headers, is_hls, is_dash, is_playlist, playlist_id, priority, max_retries) VALUES (@id, @url, @title, @filename, @save_path, @category, @status, @file_size, @downloaded, @mime_type, @referrer, @headers, @is_hls, @is_dash, @is_playlist, @playlist_id, @priority, @max_retries)`),
    ...
    updateDownloadProgress:  db.prepare(`UPDATE downloads SET downloaded=@downloaded, speed=@speed, progress=@progress, eta_seconds=@eta, file_size=@size, updated_at=datetime('now') WHERE id=@id`),
    ...
    updateDownloadFilePath:  db.prepare(`UPDATE downloads SET file_path=@file_path, file_size=@size, downloaded=@size, updated_at=datetime('now') WHERE id=@id`),
    ...
    updatePlaylistProgress:  db.prepare(`UPDATE playlists SET completed=completed+1, updated_at=datetime('now') WHERE id=?`),
